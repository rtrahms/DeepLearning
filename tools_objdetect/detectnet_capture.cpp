...
/* Set TBD representing a segment */
typedef std::pair<string, float> Detection;
...
class DetectNet {
 public:
  ...
  std::vector<Detection> CreateDetections(const cv::Mat& img, int N = 5);
...
std::vector<Detection> DetectNet::CreateDetections(const cv::Mat& img, int N) {
...
int main(int argc, char** argv) {
  ...
  DetectNet detectNet(model_file, trained_file, mean_file, label_file);
  ...
  if (!cap.isOpened())
	return -1;
  	...
  	for (;;) 
  	{
  		...
  		//std::vector<Detection> detections = detectNet.CreateDetections(inputImg, 21);

  		cv::Mat detectedImg = inputImg;

		//cv::Point org1(10, 380);
		//Display_Text(classifyImg,"Deep Learning Stats:",org1);

		/* Print the top N predictions. From Classifier code */	
		/*	
		  int maxPredictions = 2;
		  int numPredictions = (predictions.size() > maxPredictions) ? maxPredictions 
									     : predictions.size();
		  ...
		  for (size_t i = 0; i < numPredictions; ++i) {
		    ...
		    int y = 400 + (i * 20);
		    cv::Point org(x, y);
		    Display_Text(classifyImg, p.first, org);
		    cv::Point org2(400, y);
		    char str[20];
		    sprintf(str,"%.2f",p.second);
		    Display_Text(classifyImg, str, org2);
		  }
		*/
