...
void exportTrainingData()
{
    ...
    string imgFileName = trainingImagePath;
    imgFileName.append(buf);
    imgFileName.append(".jpg");

    string labelFileName = trainingLabelPath;
...
int main(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
	cout << "Usage: " << argv[0] << " <classes_file> <training_file_prefix>" << endl;
	return 0;
    }
    ...
    readInClasses(argv[1]);
    printClasses();

    trainingFilenamePrefix = argv[2];
    ...
    while(1){
	    ...
	    if (c != -1) {
		    ...
		    // space pauses file storage
		    if(c==' ')
		    {
			// toggle image writing
